...
function jg_post_thumbnail_src($size = 'thumbnail') { //get the src of first image
  ...
  if(preg_match_all('/<img.+src=[\'"]([^\'"]+)[\'"].*>/i', $post->post_content, $matches)){
	...
	if (isset($featured_image)) { // author sets a specific Featured Image
		return $featured_image[0];
	} elseif ($custom_image) { // theme has a Custom Field for "custom_image_value" and author sets the url
		return $custom_image; 
	} elseif ($image_url) { // theme has a Custom Field for "image_url" in an infographic post and author sets the url
		return $image_url; 
	} elseif (isset($atturl)) { // author has uploaded various image attachments, and may or may not have put them in the content
		return $atturl[0]; 
...
function jg_post_thumbnail($size = 'thumbnail'){ //creates an img tag for use in post lists
	if ($src = jg_post_thumbnail_src($size)) {
		if($src == DEFAULT_PHOTO){ $class = ' default-photo'; } else { $class = ''; }
		echo '<a href="'. get_permalink() .'" title="Permanent Link to '. get_the_title() .'" class="post-thumb' . $class . '"><img src="'. $src . '" alt="'. get_the_title().'"></a>';
	}
}
?>